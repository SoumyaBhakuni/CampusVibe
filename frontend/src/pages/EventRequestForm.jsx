...
export default function EventDashboard() {
  ...
  const fetchEventAndLeaderboard = useCallback(async () => {
    try {
      ...
      // --- Second, if it has a leaderboard, fetch it ---
      if (eventData.hasLeaderboard) {
        // This is our new public endpoint
        ...
        if (leaderboardRes.ok) {
          ...
          setLeaderboard([]); // Set to empty array on fail
        }
      }
  ...
  }, [id]); // This callback depends on the event ID

  useEffect(() => {
    fetchEventAndLeaderboard();
  }, [fetchEventAndLeaderboard]); // Run on mount

  // --- 2. ROLE & PERMISSION LOGIC ---
  const isOwner = user && event && event.Organizer && 
                  (user.role === 'Admin' || user.role === 'EventAdmin' || user.email === event.Organizer.email); // Updated to include EventAdmin
  ...
  const handleDeleteEvent = async () => {
    ...
    try {
      ...
      // This route is now also accessible by EventAdmin via our backend middleware logic
            ...
            {event.hasLeaderboard && (
              <div className="bg-white/5 backdrop-blur-lg rounded-2xl p-6 border border-white/10">
                ...
                <table className="w-full text-left">
                  ...
                  <tbody>
                    {/* This .map() will now use the dynamically fetched leaderboard data */}
      ...
      <div className="space-y-2 max-h-[30vh] overflow-y-auto">
        {loading ? <p>Loading team...</p> : team.map(member => (
          <div key={member.id} className="bg-white/10 p-3 rounded flex justify-between items-center">
            <div>
              {/* TODO: This should show member.Student.name, not memberId */}
    ...
    <Modal title="Manage Leaderboard" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {/* This .map() now uses the dynamically fetched data */}
